...
int main(int argc, char* argv[])
{
    ...
    HANDLE handle;
    Decompiler* decompiler = new Decompiler(CS_ARCH_X86);
    try
    {
        ...
        spdlog::info("Waiting for process {}. . .", procName);
        handle = Memory::WaitForProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, procName).first;

        if (handle == INVALID_HANDLE_VALUE || !handle) {
            spdlog::error("Error getting handle for process: {}", procName);
        }
        else {
            GeneralHookHandler::scanForHooks(procName, handle, decompiler, loadlibs, ignordiff);
        }
